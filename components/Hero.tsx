...
function StatCard({ num, label }: { num: string; label: string }) {
  return (
    <div className="border-l-4 border-blue pl-4 py-2 bg-white/90 shadow-md pr-6">
  ...
  return (
    <section className="min-h-screen grid grid-cols-1 md:grid-cols-2 items-center px-6 md:px-12 pt-32 pb-16 gap-12 relative overflow-hidden bg-white">
      ...
      <div className="relative z-10">
        <p className="text-[0.7rem] tracking-[0.22em] uppercase text-text-mid mb-5">
          {t.hero.eyebrow}
        </p>
        <h1 className="font-display text-4xl md:text-5xl lg:text-6xl font-normal leading-tight mb-6">
          {t.hero.titleStart}{' '}
        ...
        <p className="text-[0.95rem] text-text-mid max-w-[46ch] mb-10 leading-relaxed">
          {t.hero.sub}
        </p>
      ...
      {/* Direita — stats */}
      <div className="relative z-10 flex flex-col gap-4 md:pl-8">
        <StatCard num="98%" label={t.stats.fluency} />
        <StatCard num="5+" label={t.stats.years} />
        <StatCard num="100%" label={t.stats.personalized} />
        <StatCard num="100+" label={t.stats.students} />
      </div>
    </section>
  );
}