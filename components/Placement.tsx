        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl mx-auto mb-14 text-left">
          {t.placement.steps.map((step, i) => (
            <RevealSection key={step.num} delay={delays[i]}>
              <div className="bg-white/8 border border-white/15 rounded-sm p-8 h-full">
                <div className="font-display text-4xl font-bold text-white/20 leading-none mb-4">{step.num}</div>
                <div className="font-display text-lg font-bold text-white mb-2">{step.title}</div>
