    ...
    <section id="metodologia" className="bg-blue px-6 md:px-12 py-24">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-0 border border-white/20 rounded-sm overflow-hidden">
        {t.methodology.items.map((item, i) => (
          <div
            key={item.num}
            className={`p-10 bg-white/7 hover:bg-white/14 transition-all duration-300 hover:-translate-y-1 ${i < t.methodology.items.length - 1 ? 'border-b md:border-b-0 md:border-r border-white/20' : ''}`}
          >
            <div className="font-display text-6xl font-bold text-white/15 leading-none mb-5">
              {item.num}
            </div>
